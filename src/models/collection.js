...
module.exports = {
  getAllCollections: () => new Promise((resolve, reject) => {
    connection.query('SELECT pengumpulan.id,user.id as id_user, user.nama as nama_user,user.no_telp, user.email as email_user, user.no_telp as phone_user, pengumpulan.tanggal,pengumpulan.waktu,pengumpulan.pesan,pengumpulan.alamat,pengumpulan.total_minyak,status.id as id_status,status.status,mitra.nama as nama_mitra,pengumpulan.date_added,pengumpulan.id_kota,kota.kota,pengumpulan.date_updated FROM pengumpulan INNER JOIN user ON pengumpulan.id_user = user.id INNER JOIN status ON pengumpulan.id_status = status.id LEFT JOIN mitra ON pengumpulan.id_mitra =  mitra.id INNER JOIN kota ON kota.id = pengumpulan.id_kota ORDER BY id_status,date_added', (error, result) => {
      if (!error) {
        resolve(result);
      } else {
